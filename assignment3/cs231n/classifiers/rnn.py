...
class CaptioningRNN:
    ...
    def __init__(
        ...
        - input_dim: Dimension D of input image feature vectors. D
        - wordvec_dim: Dimension W of word vectors. W
        - hidden_dim: Dimension H for the hidden state of the RNN. H
    ...
    def loss(self, features, captions):
        ...
        captions_in = captions[:, :-1] #except last word
        captions_out = captions[:, 1:]  #except first word (not START token)

        # You'll need this
        mask = captions_out != self._null # not NULL
        ...
        #forward
        h0 = features.dot(W_proj) + b_proj # (N,H) features:input images features
        x, cache_we = word_embedding_forward(captions_in, W_embed)  #(N,T,W)
        ...
        dW_proj = features.T.dot(dh0) #(D,N)x(N,H) -> (D,H)
        db_proj = dh0.sum(axis=0) #(H,) 세로 합

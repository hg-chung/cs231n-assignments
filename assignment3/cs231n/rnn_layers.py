...
def rnn_step_backward(dnext_h, cache):
    ...
    dsum = dnext_h * (1 - next_h**2)    # dtanh = 1 - tanh**2
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0.copy()
    next_h = None
    h = np.zeros((x.shape[1], x.shape[0], h0.shape[1])) #(T,N,H)
    cache = []
    x = x.transpose(1,0,2)  #(N,T,D) -> (T,N,D)
    
    for T in range(x.shape[0]): # repeat with T times
      next_h, data = rnn_step_forward(x[T], prev_h, Wx, Wh, b)
      cache.append(data)
      h[T] = next_h
      prev_h = next_h

    h = h.transpose(1,0,2)  #(T,N,H) -> (N,T,H)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):  #거꾸로 역전파
      data = cache[t] # recall cache[t]
      dh_sum = dh[:,t,:] + dprev_h  # sum of  dh and derivative of previous h
...
def word_embedding_backward(dout, cache):
    ...
    dout_flat = dout.reshape(-1, dout.shape[2]) #(N*T, D)

    np.add.at(dW, x_flat, dout_flat)  #update dW
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dao = do * o*(1-o) # diff of sigmoid x/(1-x)
    ...
    daf = df * f * (1-f)
    dai = di * i * (1-i)
    dag = dg * (1 - g**2) # diff of tanh (1-tanh^2)

    da = np.hstack((dai, daf, dao, dag))

    dx, dWx, db = affine_backward(da, (x, Wx, b))
    dprev_h, dWh, db = affine_backward(da, (prev_h, Wh, b))
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    x = x.transpose(1,0,2)  #(N,T,D) -> (T,N,D)
    prev_c = np.zeros((h0.shape[0], h0.shape[1])) # (N,H)
    
    for T in range(x.shape[0]): # repeat with T times
      next_h, next_c, data = lstm_step_forward(x[T], prev_h, prev_c, Wx, Wh, b) # lstm step 
      cache.append(data) # store cache data
      h[T] = next_h # store hidden state
      prev_c = next_c # cell state 
      prev_h = next_h # hideen state
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):  #거꾸로 역전파
      data = cache[t] # recall cache[t]
      dh_sum = dh[:,t,:] + dprev_h  # sum of  dh and derivative of previous h
      dx_, dprev_h, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dh_sum, dprev_c, data)
      dx[:,t,:] = dx_
      dWx += dWx_
      dWh += dWh_
      db += db_
